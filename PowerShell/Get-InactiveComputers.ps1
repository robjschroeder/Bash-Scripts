...
$daysInactive = 7
$time = (Get-Date).AddDays(-($daysInactive))
Get-ADComputer -Filter {LastLogonTimeStamp -lt $time} -SearchBase "ou=$ou,ou=computers,ou=awc,dc=adroot,dc=azwestern,dc=edu" -Properties LastLogonTimeStamp | Sort Name |
